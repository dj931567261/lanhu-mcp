---
name: lanhu-rn-plan
description: 输入蓝湖设计图详情 URL（含 image_id），使用 lanhu MCP image_id 精确链路输出 React Native TSX 页面规划与样式草稿；annotations 为主路径，单位与圆角归一在 skill 内完成，schema 仅失败兜底，资源统一落盘到 src/assets/lanhu/<screen>/。
---

# 蓝湖 → React Native TSX（image_id 直连 + annotations 主路径 + skill 归一）

## Skill 执行优先级（硬门禁）

当用户消息显式包含 `$lanhu-rn-plan`（或明确点名本 skill）时，必须执行以下规则：

1. **本 skill 规则最高优先级**：
   - 禁止回退到通用 UI 转换文档作为主流程。
   - 禁止输出 XML、Compose、Flutter、SwiftUI。
...
- 或 key=value 多行：
  ...
  - `HOST_ACTIVITY=...`（可选）
  - `HOST_FRAGMENT=...`（可选）
  - `PRESENTATION=auto|inline|screen|modal|bottom_sheet|popup`

### 输入硬要求

- `LANHU_URL` 必须是设计图详情链接并包含 `image_id`（`detailDetach?...&image_id=...`）。
- 为兼容既有调用协议，保留 `HOST_ACTIVITY` / `HOST_FRAGMENT` 字段名；在 RN 场景中仅作为挂载点线索，不生成 Android 代码。
...
- `annotations.style.border_radius` 是优先可消费圆角值：
  - 统一圆角输出：`borderRadius`
  - 分角圆角输出：`borderTopLeftRadius` / `borderTopRightRadius` / `borderBottomRightRadius` / `borderBottomLeftRadius`
  - 仅当 `annotations.style.border_radius` 缺失时，才允许从图层样式补推
- 所有 `*_raw` 字段都视为原始源数据：
  - 禁止直接输出到 RN 代码
  - 必须先由 skill 完成单位归一、结构归一，再写入 TSX / `StyleSheet`
- `annotations.unit = dp` 时：
  - 位置、宽高、边框、阴影、间距、圆角直接视为 React Native 逻辑像素
  - 文本尺寸直接输出为归一后的数值
  - 禁止再次执行 `411/750` 缩放
- 文本规则：
  - `font_size` -> `fontSize`
...
- 禁止把 `*_raw`、`px`、未归一的 schema 原值直接塞进 TSX / `StyleSheet.create`
...
5. **适配策略统一**：
   ...
   - SafeAreaView should NOT be used to replace design-specified spacing
...
- 所有 schema 原值先落入 `*_raw`，再完成归一后才能输出到 RN 代码
...
1. **仅 TSX + StyleSheet**：
   - 阶段 2 的代码产出只能是 TSX 文件内容与 `StyleSheet.create(...)`。
   - 禁止输出 XML、Compose、Flutter、伪 DSL。
2. **布局组件优先级**：
   - 优先 `View` / `Text` / `Image` / `ScrollView` / `Pressable`
   - 默认使用 flex 布局
   - 只有重叠场景才允许 `position: absolute`
3. **圆角输出规则**：
   - 统一圆角：`borderRadius`
   - 分角圆角：各自对应的 `border*Radius`
   - 禁止输出未归一的圆角数组或 `*_raw`
4. **资源落盘规则**：
   - 所有切图、图标、背景资源统一放到：`src/assets/lanhu/<screen>/`
   - 资源命名使用语义化名称，例如：`ic_close.png`、`img_banner.webp`、`bg_card.png`
5. **资源使用约束**：
   - 常规资源使用 `Image` 或 `ImageBackground`
   - 仅明显纯色基础几何形才允许直接用 `View` 样式绘制

6. **切图格式约束**：
   - 切图默认输出 `webp`（透明或质量异常可回退 `png`，并在规格表标注原因）
   - 所有栅格切图统一落盘到：`src/assets/lanhu/<screen>/`
7. **icon / 切图来源约束（强制）**：
   - 除"明显纯色填充的基础几何形（无渐变/阴影/描边/纹理）"外，icon 不得手动用 SVG/Canvas 绘制
   - 常规 icon 必须直接使用 `lanhu_get_design_slices(url, image_id)` 返回的资源
   - 当 slices 提供了某个区域的完整合成切图（如 `bg`、`编组`、`layer-group` 类型的切图，且面积覆盖该区域 >= 80%），必须优先使用切图 `<Image source={require(...)}/>` 作为背景，禁止用 annotations 的纯色/渐变去模拟多层叠加效果
8. **字体/颜色（可读性优先）**：
   - 禁止为单个控件创建额外的 StyleSheet 常量
   - 字体属性完全相同在同一界面出现 `>=2` 次才允许抽成共享样式，否则一律 inline
   - 颜色默认使用文件级常量；同色 `>=3` 次或全局语义色才建议归并到 theme/token

## RN UI 适配优化（必须执行）

1. **根布局适配**：
   - 页面根节点优先使用 `SafeAreaView`（仅用于系统安全区适配，SafeAreaView should NOT be used to replace design-specified spacing）
   - 主内容区优先通过 `useWindowDimensions()` 获取宽度，再结合 `maxWidth`、`alignSelf: 'center'`、百分比宽度控制版心
   - 禁止把整页宽高写死成 `375/812` 一类常量
2. **滚动与键盘避让**：
   - 只要垂直内容有超出风险，优先使用 `ScrollView` 或 `FlatList`
   - 输入页、表单页优先使用 `KeyboardAvoidingView`
   - `contentContainerStyle` 需预留底部操作区和安全区空间（SafeAreaView should NOT be used to replace design-specified spacing）
3. **大屏与横屏策略**：
   - 平板或横屏优先“主栏限宽 + 居中”，禁止整体缩放
   - 仅当视觉明确要求双列时，才允许拆成左右栏
4. **弹性布局规则**：
   - 默认使用 flex 布局和百分比/自适应宽度
   - 只有重叠场景才允许 `position: absolute`
   - 禁止用一组绝对定位值拼完整主页面
5. **文本与图片适配**：
   - 长文本必须给出 `numberOfLines` / `ellipsizeMode`
   - 图片必须显式声明 `resizeMode`
   - 比例敏感资源优先补 `aspectRatio`
6. **弹层与底部区域**：
   - `modal` / `bottom_sheet` 需额外处理安全区和底部留白（SafeAreaView should NOT be used to replace design-specified spacing）
   - 底部主按钮不得被刘海、手势条或键盘遮挡
...
2. **优先使用 `measurements.sibling_spacings`**（如果可用）：
   - 直接读取 `gap_to_next` 作为 `marginTop/marginLeft` 或 `gap` 属性
   - 当 `layout_direction = horizontal` → 使用 `flexDirection: 'row'`
   - 当 `layout_direction = vertical` → 使用 `flexDirection: 'column'`
   - 当 `layout_direction = stack` → 使用绝对定位叠加
3. **禁止用通用系统适配方案替代设计稿间距**：
   - 禁止用 `SafeAreaView` 替代设计稿中已标注的具体间距
...
- **阶段 1**：只输出“选中画板 + 默认承载方式 + 预期产出文件清单”。
  ...
  - 不输出 slices 列表
  - 不输出接入说明
- **阶段 2**：只输出「A) 审计区 + B) 规格表 + C) React Native 文件内容 + D) 资源清单」。
...
4. 默认 `PRESENTATION` 规则：
   ...
   - `screen`：明确“独立整屏页面/导航入口/深链页”
   - 否则 `inline`
5. 阶段 1 输出：
   - `SELECTED: image_id=... name=... size_dp=...`
   - `PRESENTATION: <value>（一句理由）`
   - `FILES: src/screens/<Screen>.tsx, src/assets/lanhu/<screen>/...`
...
- `asset_format(webp/png) | asset_output_dir(src/assets/lanhu/<screen>/) | asset`

### C) React Native 文件内容约束

- 只允许输出 TSX 文件内容与 `StyleSheet.create(...)`
- 优先使用函数组件 + 局部 `StyleSheet.create(...)`
- 默认使用 flex 布局
- 需要根据页面风险显式体现 `SafeAreaView`（仅用于系统安全区，SafeAreaView should NOT be used to replace design-specified spacing）、`useWindowDimensions`、滚动容器、键盘避让策略
- 资源引用使用：`require('../assets/lanhu/<screen>/...')` 或等价相对路径
- 禁止输出未归一的 `*_raw`、schema 原值、XML 属性名
- 禁止输出路由接入代码、状态管理接线或验收说明

### D) 资源清单最少字段

- `asset_root=src/assets/lanhu/<screen>/`
- `asset_require_strategy=require|import`
- `asset_format(webp/png) | asset_output_dir(src/assets/lanhu/<screen>/) | asset`

---
name: lanhu-uniapp-plan
description: 输入蓝湖设计图详情 URL（含 image_id），使用 lanhu MCP image_id 精确链路输出 uni-app 页面规划与代码草稿；annotations 为主路径，单位与圆角归一在 skill 内完成，schema 仅失败兜底，资源统一落盘到 static/lanhu/<screen>/ 并补齐 pages.json 页面片段。
---

# 蓝湖 → uni-app（image_id 直连 + annotations 主路径 + skill 归一）

## Skill 执行优先级（硬门禁）

当用户消息显式包含 `$lanhu-uniapp-plan`（或明确点名本 skill）时，必须执行以下规则：

1. **本 skill 规则最高优先级**：
   - 禁止回退到通用 UI 转换文档作为主流程。
   - 禁止输出原生小程序、Vue Web、Taro、React Native、Flutter。
...
- 或 key=value 多行：
  ...
  - `HOST_ACTIVITY=...`（可选）
  - `HOST_FRAGMENT=...`（可选）
  - `PRESENTATION=auto|embedded|page|dialog|drawer|bottom_sheet|popup|tab_page`

### 输入硬要求

- `LANHU_URL` 必须是设计图详情链接并包含 `image_id`（`detailDetach?...&image_id=...`）。
- 为兼容既有调用协议，保留 `HOST_ACTIVITY` / `HOST_FRAGMENT` 字段名；在 uni-app 场景中仅作为宿主页面或挂载点线索，不生成 Android 代码。
...
- `annotations.style.border_radius` 是优先可消费圆角值：
  - 统一圆角输出：`border-radius: ...rpx;`
  - 分角圆角输出：各自对应的样式四角圆角
  - 仅当 `annotations.style.border_radius` 缺失时，才允许从图层样式补推
- 所有 `*_raw` 字段都视为原始源数据：
  - 禁止直接输出到 uni-app 代码
  - 必须先由 skill 完成单位归一、结构归一，再写入 `.vue`
- `annotations.unit = dp` 时：
  - 默认使用 411 基线回推到 750 设计基线：`rpx = round(dp * 750 / 411)`
  - `font_size` / `line_height` / `letter_spacing` 默认也按 rpx 输出
  - 禁止再次执行 `411/750` 缩放
- `web_schema_fallback` 原始 px 时：
  - 默认直接按 750 设计稿基线消费：`raw_px@750 -> rpx@750`
- 仅当用户明确要求 H5-only 且项目已有 px 体系时，才允许切换为 px
...
- 禁止把 `*_raw`、未归一的 schema 原值直接塞进 uni-app 代码
...
- 所有 schema 原值先落入 `*_raw`，再完成归一后才能输出到 uni-app 代码
...
1. **仅 uni-app 页面**：
   - 阶段 2 的代码产出只能是 uni-app `.vue` 页面文件内容与 `pages.json` 页面片段。
   - 禁止输出原生小程序四件套、Vue Web 构建配置、Taro、React。
2. **布局组件优先级**：
   - 优先 `view` / `text` / `image` / `button` / `input` / `scroll-view`
   - 默认使用 flex 布局
...
3. **圆角输出规则**：
   ...
   - 禁止输出未归一的圆角数组或 `*_raw`
4. **资源落盘规则**：
   - 所有切图、图标、背景资源统一放到：`static/lanhu/<screen>/`
   - 资源命名使用语义化名称，例如：`ic-close.png`、`img-banner.webp`、`bg-card.png`
5. **资源使用约束**：
   - 常规资源使用相对路径或 static 资源路径
   - 仅明显纯色基础几何形才允许直接用样式绘制

6. **切图格式约束**：
   - 切图默认输出 `webp`（透明或质量异常可回退 `png`，并在规格表标注原因）
   - 所有栅格切图统一落盘到：`static/lanhu/<screen>/`
7. **icon / 切图来源约束（强制）**：
   - 除"明显纯色填充的基础几何形（无渐变/阴影/描边/纹理）"外，icon 不得手动用 CSS/Canvas 绘制
   - 常规 icon 必须直接使用 `lanhu_get_design_slices(url, image_id)` 返回的资源
   - 当 slices 提供了某个区域的完整合成切图（如 `bg`、`编组`、`layer-group` 类型的切图，且面积覆盖该区域 >= 80%），必须优先使用切图 `<image>` 或 `background-image` 作为背景，禁止用 annotations 的纯色/渐变去模拟多层叠加效果
8. **字体/颜色（可读性优先）**：
   - 禁止为单个元素创建额外的 CSS class
   - 字体属性完全相同在同一界面出现 `>=2` 次才允许抽成共享 class，否则一律 inline style
   - 颜色默认使用 CSS 变量或 scss 变量；同色 `>=3` 次或全局语义色才建议归并到公共样式

## uni-app UI 适配优化（必须执行）

1. **根布局适配**：
   - 页面根节点优先使用正常文档流和 flex 容器
   - 主内容区优先结合最大宽度思路控制版心
   - 禁止整页缩放
2. **安全区与系统栏**：
   - 底部操作区必须为安全区留白
   - 顶部自定义导航按需为状态栏高度留白
3. **滚动与溢出**：
   - 只要垂直内容有超出风险，优先使用页面滚动或 `scroll-view`
   - 禁止把底部按钮写死到安全区外
4. **大屏与横屏策略**：
   - 大屏优先“居中主栏 + 最大宽度约束”
   - 横屏优先重排容器，不做整页缩放
...
2. **优先使用 `measurements.sibling_spacings`**（如果可用）：
   - 直接读取 `gap_to_next` 作为 `margin-top/margin-left` 值（rpx 或 px）
...
1. **当 `layout_tree` 可用且有层级时，必须按树结构组织模板**：
   - 树的每个 group 节点对应一个容器 `<view>` 或语义化组件
...
1. **边框**：读取 `style.borders_parsed[]`
   - 输出：`border: {width}rpx solid {color}`
2. **阴影**：读取 `style.shadows[]`
   - 输出：`box-shadow: {offsetX}rpx {offsetY}rpx {blur}rpx {color}`
3. **圆角**：`border_radius` 和 `border_radius_detail_raw` 已转为 dp
   - 禁止对圆角值再次执行缩放
   - 输出：`border-radius: {value}rpx` 或分角写法
...
- **阶段 1**：只输出“选中画板 + 默认承载方式 + 预期产出文件清单”。
  ...
  - 不输出 slices 列表
  - 不输出接入说明
- **阶段 2**：只输出「A) 审计区 + B) 规格表 + C) uni-app 文件内容 + D) 资源清单」。
...
4. 默认 `PRESENTATION` 规则：
   ...
   - `tab_page`：明确“tab 页入口/底部 tab 页面”
   - `page`：明确“独立整屏页面/导航入口/深链页”
   - 否则 `embedded`
5. 阶段 1 输出：
   - `SELECTED: image_id=... name=... size_dp=...`
   - `PRESENTATION: <value>（一句理由）`
   - `FILES: pages/<screen>/<screen>.vue, pages.json(page snippet), static/lanhu/<screen>/...`

## 阶段 2（审计 + 规格 + uni-app + 资源）
...
- `asset_format(webp/png/svg) | asset_output_dir(static/lanhu/<screen>/) | asset`

### C) uni-app 文件内容约束

- 只允许输出页面 `.vue` 文件内容与 `pages.json` 页面片段
- 默认输出 `<template>`、`<script setup lang="ts">`、`<style scoped>`
- 需要根据页面风险显式体现滚动容器、安全区、吸底操作区策略
- 资源引用使用 static 资源路径或相对路径
- 禁止输出未归一的 `*_raw`、schema 原值、原生小程序四件套、构建配置、接入说明或验收说明

### D) 资源清单最少字段

- `asset_root=static/lanhu/<screen>/`
- `pages_json_snippet=required`
- `asset_import_strategy=static_path|relative_path`
- `asset_format(webp/png/svg) | asset_output_dir(static/lanhu/<screen>/) | asset`

### 兜底路径补充字段（仅 fallback）

- `design_baseline_width=750`
- `normalized_unit=rpx`
- `conversion_rule=raw_px@750 -> rpx@750`
- `schema_version_id`
- `schema_source_url`

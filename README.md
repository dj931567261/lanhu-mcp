...
在 Codex 的配置文件 `config.toml`（通常位于 `~/.codex/config.toml`）中添加 MCP 配置，添加后每次对话都可以直接使用蓝湖 MCP 工具。
...
**Windows（方式一：使用 Git Bash，推荐）：**
```toml
[mcp_servers.Lanhu-mcp]
type = "stdio"
command = "D:\\Git\\bin\\bash.exe"
args = ["C:/path/to/lanhu-mcp/run_lanhu_mcp_stdio.sh"]
```
...
type = "stdio"
command = "cmd"
args = ["/c", "C:\\path\\to\\lanhu-mcp\\run_lanhu_mcp_stdio.bat"]
```

> `config.toml` 文件位置：macOS / Linux 为 `~/.codex/config.toml`，Windows 为 `%USERPROFILE%\.codex\config.toml`。
